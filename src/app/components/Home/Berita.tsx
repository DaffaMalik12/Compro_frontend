...
export default function ProgramSection({ berita }: Props) {
  // Saya juga memperbaiki typo "blue" di luar tag Link
  return (
    <div className=" py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {berita.map((beritaItem) => (
            ...
            <div key={beritaItem.id} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col transform hover:-translate-y-1 transition-transform duration-300">
              ...
              {/* 2. Konten di dalam card juga menjadi flex container */}
              <div className="p-6 flex flex-col flex-grow">
                {/* 3. Wrapper untuk teks diberi flex-grow agar mendorong tombol ke bawah */}
                <div className="flex-grow">
                  <p className="text-sm text-blue-600 font-semibold">{beritaItem.kategori}</p>
